...
def _upload_file(job_node, source_name, destination_name=None):
...
def _upload_callback_data(data, job_node, storage):
    # create temporary file to store callback data with gzip
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        with open(os.path.join(tmp_dir, filename), 'w') as f:
            ...
            f.write(serjson)
        os.chdir(tmp_dir)
        return _upload_file(job_node, filename)
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        with gzip.open(os.path.join(tmp_dir, 'lava_log.txt.gz'), 'wt') as f:
            ...
            if f.tell() == 0:
                return None
        os.chdir(tmp_dir)
        return _upload_file(job_node, 'lava_log.txt.gz', 'log.txt.gz')
